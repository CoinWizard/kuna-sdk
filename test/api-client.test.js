...
describe('Kuna API v2 Client', () => {
    const badUrl = 'https://kuna.io.2/api/v2';
    describe('constructor with bad url ' + badUrl, () => {
        var kuna = new Kuna.KunaApiClient('', '', badUrl);
        it('getTicker(btcuah)', async () => {
            try {
                await kuna.getTicker('btcuah');
                new Error('should throw exception with bad url ' + badUrl);
            } catch (error) {
            }
        });
    });
    ...
    describe('default constructor', () => {
        ...
        it('getTimestamp()', async () => {
            const response = await kuna.getTimeStamp();
        ...
        it('getTickers()', function(done) {
            kuna.getTickers().then(function(response) {
                ...
                var responseMarkets = Object.keys(
                    _.mapKeys(response, 'market'));
                var kunaApiMarkets = Object.keys(Markets);

                var missingMarkets = _.difference(responseMarkets,
                    kunaApiMarkets);

                if (missingMarkets.length > 0) {
                    done(
                        new Error('markets.ts[kunaMarketMap] missing supported markets=' +
                            missingMarkets));
                    return;
                }
                var notSupportedMarkets = _.difference(kunaApiMarkets,
                    responseMarkets);
                if (notSupportedMarkets.length > 0) {
                    console.warn(
                        '    !!!! kunaMarketMap has not supported markets=' +
                        notSupportedMarkets + ' !!!!',
                    );
                }
                done();
        ...
        it(`getOrderBook(${Markets.btcuah.key})`, (done) => {
            kuna.getOrderBook(Markets.btcuah.key).then((response) => {
                ...
                expect(+response['asks'][0][0]).
                    toBeLessThan(+response['asks'][1][0]);
                expect(+response['bids'][0][0]).
                    toBeGreaterThan(+response['bids'][1][0]);
                done();
            }).catch(function(error) {
                done(error);
            });
        ...
        it(`getTrades(${Markets.btcuah.key})`, (done) => {
            kuna.getTrades(Markets.btcuah.key).then((response) => {
                expect(response.length).toBeGreaterThan(0);
    ...
    describe('constructor with read only secret key - no trades permissions', () => {
            ...
            var kuna = new Kuna.KunaApiClient(
                '50VExXAENWUIPv9hWIEngSWYfevwxtsnNnPx3cxH',
                'vzveddrDHgnL9CJnTKRIR1UsI8fxX9eJ8KQ4SlBS');
            ...
            it('getUserOrders() with no permissions', function(done) {
                kuna.getUserOrders(Markets.btcuah.key).then(function(response) {
                        ...
                        'support to fail this request as we do not have trade permissions'));
                }).catch(function() {
                    // console.log(error.response.data);
                    done();
                });
    ...
    describe('constructor with trade secret key - with trades permissions',
        function() {
            var kuna = new Kuna.KunaApiClient(
                'FWVmBtxETtEdqlYRZxfcfboJpyuZxO3BV6nPNVeM',
                'wsSWP1AeAo0mvr18t0CcBXPLhT5RJo3xLdgxQKXn');

            it('newOrder()+getUserOrders()+cancelOrder()', function(done) {
                kuna.newOrder('sell', 0.0001, Markets.wavesuah.key, 300000.1).
                    then(function(response) {
                        expect(response.id).toBeGreaterThan(0);

                        kuna.getUserOrders(Markets.wavesuah.key).
                            then(function(response2) {
                                assert.equal(typeof response, 'object');
                                expect(response2.length).toBeGreaterThan(0);
                                // console.log("user orders=", response);
                                kuna.cancelOrder(response.id).
                                    then(function(response2) {
